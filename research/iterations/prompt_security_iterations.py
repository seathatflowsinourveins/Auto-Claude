"""
PROMPT SECURITY ITERATIONS - LLM Attack & Defense
=================================================
Prompt injection, jailbreaks, adversarial attacks
"""
...
SECURITY_TOPICS = [
    # Prompt Injection
    {"topic": "Prompt injection attacks: direct, indirect, nested", "area": "injection"},
    {"topic": "Indirect prompt injection: document, email, web", "area": "injection"},
    {"topic": "Prompt injection defense: detection, prevention", "area": "injection"},
    {"topic": "Input sanitization: filtering malicious prompts", "area": "injection"},

    # Jailbreaks
    {"topic": "Jailbreak techniques: DAN, roleplay, encoding", "area": "jailbreak"},
    {"topic": "Jailbreak detection: pattern matching, classifiers", "area": "jailbreak"},
    {"topic": "Universal adversarial prompts: transferable attacks", "area": "jailbreak"},
    {"topic": "Jailbreak mitigation: system prompts, fine-tuning", "area": "jailbreak"},

    # Data Security
    {"topic": "Training data extraction: memorization attacks", "area": "data"},
    {"topic": "PII leakage prevention: output filtering", "area": "data"},
    {"topic": "Membership inference: detecting training data", "area": "data"},
    {"topic": "Data poisoning: training data attacks", "area": "data"},

    # Adversarial ML
    {"topic": "Adversarial examples for LLMs: perturbations", "area": "adversarial"},
    {"topic": "Model stealing: extracting model capabilities", "area": "adversarial"},
    {"topic": "Backdoor attacks: hidden model behaviors", "area": "adversarial"},
    {"topic": "Robustness evaluation: red team testing", "area": "adversarial"},

    # Defense Strategies
    {"topic": "Defense in depth: layered security for LLMs", "area": "defense"},
    {"topic": "Constitutional AI: self-correcting safety", "area": "defense"},
    {"topic": "Sandboxing LLM outputs: isolated execution", "area": "defense"},
    {"topic": "Security monitoring: detecting attacks in production", "area": "defense"},
]
...
class SecurityExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("security", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Prompt Security Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM security research: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("security", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> SecurityResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return SecurityResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("PROMPT SECURITY ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(SECURITY_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(SECURITY_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("PROMPT SECURITY COMPLETE")
    print("="*70)
    ...
    with open("prompt_security_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to prompt_security_results.json")
    print("="*70)
