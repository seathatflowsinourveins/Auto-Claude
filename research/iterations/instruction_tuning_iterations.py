"""
INSTRUCTION TUNING ITERATIONS - Fine-tuning
==========================================
Instruction tuning, SFT, chat models
"""
...
INSTRUCTION_TUNING_TOPICS = [
    # Methods
    {"topic": "Instruction tuning: task-specific training", "area": "methods"},
    {"topic": "Supervised fine-tuning: SFT datasets", "area": "methods"},
    {"topic": "Chat fine-tuning: conversation format", "area": "methods"},
    {"topic": "FLAN: instruction-tuned models", "area": "methods"},

    # Datasets
    {"topic": "ShareGPT: conversation datasets", "area": "datasets"},
    {"topic": "Alpaca: instruction dataset", "area": "datasets"},
    {"topic": "OpenAssistant: human feedback data", "area": "datasets"},
    {"topic": "Dolly: instruction-following data", "area": "datasets"},

    # Techniques
    {"topic": "LoRA: low-rank adaptation", "area": "techniques"},
    {"topic": "QLoRA: quantized LoRA", "area": "techniques"},
    {"topic": "Full fine-tuning: all parameters", "area": "techniques"},
    {"topic": "Adapter tuning: lightweight modules", "area": "techniques"},

    # Infrastructure
    {"topic": "Axolotl: fine-tuning framework", "area": "infrastructure"},
    {"topic": "Unsloth: fast fine-tuning", "area": "infrastructure"},
    {"topic": "LLaMA-Factory: efficient training", "area": "infrastructure"},
    {"topic": "DeepSpeed: distributed fine-tuning", "area": "infrastructure"},

    # Evaluation
    {"topic": "Instruction following: quality metrics", "area": "evaluation"},
    {"topic": "Chatbot evaluation: conversation quality", "area": "evaluation"},
    {"topic": "Task performance: downstream tasks", "area": "evaluation"},
    {"topic": "Safety evaluation: alignment checks", "area": "evaluation"},
]

@dataclass
class InstructionTuningResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class InstructionTuningExecutor:
    def __init__(self):
        self.exa = None
        self.qdrant = None
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("instruction_tuning", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Instruction Tuning Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Instruction tuning: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("instruction_tuning", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> InstructionTuningResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return InstructionTuningResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("INSTRUCTION TUNING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(INSTRUCTION_TUNING_TOPICS)}")
    print("="*70)

    executor = InstructionTuningExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(INSTRUCTION_TUNING_TOPICS, 1):
        try:
    ...
    with open("instruction_tuning_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to instruction_tuning_results.json")
    print("="*70)
