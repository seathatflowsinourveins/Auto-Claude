...
class VectorExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("vectordb", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Vector Database Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Vector database implementation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("vectordb", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> VectorResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return VectorResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("VECTOR DATABASES ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(VECTOR_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(VECTOR_TOPICS, 1):
        try:
    ...
    with open("vector_databases_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to vector_databases_results.json")
    print("="*70)
