"""
DOCUMENT AI ITERATIONS - Document Understanding
===============================================
...
DOCUMENT_AI_TOPICS = [
    # OCR
    {"topic": "OCR: optical character recognition", "area": "ocr"},
    {"topic": "Tesseract: open source OCR", "area": "ocr"},
    {"topic": "Textract: AWS document AI", "area": "ocr"},
    {"topic": "Document AI: Google Cloud", "area": "ocr"},

    # Layout
    {"topic": "Layout analysis: document structure", "area": "layout"},
    {"topic": "Table extraction: structured data", "area": "layout"},
    {"topic": "Form recognition: field extraction", "area": "layout"},
    {"topic": "Document segmentation: regions", "area": "layout"},

    # Models
    {"topic": "LayoutLM: document understanding", "area": "models"},
    {"topic": "Donut: document transformer", "area": "models"},
    {"topic": "DocVQA: document question answering", "area": "models"},
    {"topic": "Nougat: PDF to text model", "area": "models"},

    # Processing
    {"topic": "PDF parsing: text extraction", "area": "processing"},
    {"topic": "Invoice processing: automation", "area": "processing"},
    {"topic": "Contract analysis: legal AI", "area": "processing"},
    {"topic": "Receipt scanning: expense AI", "area": "processing"},

    # Integration
    {"topic": "Document RAG: retrieval documents", "area": "integration"},
    {"topic": "Multimodal documents: vision-text", "area": "integration"},
    {"topic": "Document workflows: automation", "area": "integration"},
    {"topic": "Document search: semantic", "area": "integration"},
]
...
class DocumentAIExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("document_ai", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Document AI Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Document AI understanding: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("document_ai", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> DocumentAIResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return DocumentAIResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("DOCUMENT AI ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(DOCUMENT_AI_TOPICS)}")
    print("="*70)

    executor = DocumentAIExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(DOCUMENT_AI_TOPICS, 1):
        try:
    ...
    with open("document_ai_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to document_ai_results.json")
    print("="*70)
