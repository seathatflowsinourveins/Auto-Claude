...
class StructuredExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("structured", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Structured Output Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM structured outputs: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("structured", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> StructuredResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return StructuredResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("STRUCTURED OUTPUT ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(STRUCTURED_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(STRUCTURED_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to structured_output_results.json")
    print("="*70)
