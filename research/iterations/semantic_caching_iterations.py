...
class CachingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("caching", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Semantic Caching Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Semantic caching for LLMs: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("caching", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> CachingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return CachingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("SEMANTIC CACHING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(CACHING_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(CACHING_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("SEMANTIC CACHING COMPLETE")
    print("="*70)
    ...
    with open("semantic_caching_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to semantic_caching_results.json")
    print("="*70)
