"""
DATA GENERATION ITERATIONS - Training Data
==========================================
Data generation, augmentation, curation
"""
...
DATA_GENERATION_TOPICS = [
    # Methods
    {"topic": "LLM data generation: synthetic training", "area": "methods"},
    {"topic": "Self-instruct: automated instruction", "area": "methods"},
    {"topic": "Evol-Instruct: complexity evolution", "area": "methods"},
    {"topic": "WizardLM: instruction generation", "area": "methods"},

    # Quality
    {"topic": "Data filtering: quality control", "area": "quality"},
    {"topic": "Decontamination: benchmark leakage", "area": "quality"},
    {"topic": "Diversity sampling: coverage", "area": "quality"},
    {"topic": "Human verification: labeling", "area": "quality"},

    # Augmentation
    {"topic": "Text augmentation: paraphrasing", "area": "augmentation"},
    {"topic": "Back-translation: multilingual", "area": "augmentation"},
    {"topic": "Style transfer: domain adaptation", "area": "augmentation"},
    {"topic": "Noise injection: robustness", "area": "augmentation"},

    # Datasets
    {"topic": "OpenHermes: synthetic chat", "area": "datasets"},
    {"topic": "Capybara: curated instruction", "area": "datasets"},
    {"topic": "SlimOrca: deduplicated data", "area": "datasets"},
    {"topic": "Airoboros: GPT-4 generated", "area": "datasets"},

    # Privacy
    {"topic": "Privacy synthetic: differential privacy", "area": "privacy"},
    {"topic": "PII removal: sensitive data", "area": "privacy"},
    {"topic": "Federated data: distributed training", "area": "privacy"},
    {"topic": "Anonymization: data protection", "area": "privacy"},
]
...
class DataGenerationExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("data_generation", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Data Generation Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI training data generation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("data_generation", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> DataGenerationResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return DataGenerationResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("DATA GENERATION ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(DATA_GENERATION_TOPICS)}")
    print("="*70)

    executor = DataGenerationExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(DATA_GENERATION_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to data_generation_results.json")
    print("="*70)
