"""
TOKEN OPTIMIZATION ITERATIONS - Context & Cost Efficiency
=========================================================
Context compression, token counting, cost reduction
"""
...
TOKEN_TOPICS = [
    # Context Compression
    {"topic": "LLMLingua: prompt compression without quality loss", "area": "compression"},
    {"topic": "Selective context: keeping only relevant information", "area": "compression"},
    {"topic": "Summarization for context: condensing long documents", "area": "compression"},
    {"topic": "Recursive summarization: hierarchical context reduction", "area": "compression"},

    # Token Counting
    {"topic": "Tiktoken: accurate token counting for OpenAI models", "area": "counting"},
    {"topic": "Tokenizer differences: GPT vs Claude vs Gemini", "area": "counting"},
    {"topic": "Token budget management: staying within limits", "area": "counting"},
    {"topic": "Pre-flight token estimation: avoiding truncation", "area": "counting"},

    # Cost Reduction
    {"topic": "Prompt caching: OpenAI and Anthropic cached prompts", "area": "cost"},
    {"topic": "Batch API: reduced costs for async processing", "area": "cost"},
    {"topic": "Token-efficient prompting: minimizing input tokens", "area": "cost"},
    {"topic": "Response length control: limiting output tokens", "area": "cost"},

    # Context Management
    {"topic": "Sliding window context: managing conversation history", "area": "management"},
    {"topic": "Context prioritization: important vs optional content", "area": "management"},
    {"topic": "Dynamic context: adding/removing based on relevance", "area": "management"},
    {"topic": "Context chunking: splitting large documents", "area": "management"},

    # Advanced
    {"topic": "Attention-aware compression: preserving key information", "area": "advanced"},
    {"topic": "Token efficiency metrics: measuring cost per task", "area": "advanced"},
    {"topic": "Multi-turn optimization: efficient conversation design", "area": "advanced"},
    {"topic": "Embedding-based selection: semantic context filtering", "area": "advanced"},
]
...
class TokenExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("token", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Token Optimization Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM token optimization: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("token", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> TokenResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return TokenResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("TOKEN OPTIMIZATION ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(TOKEN_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(TOKEN_TOPICS, 1):
        try:
    ...
    with open("token_optimization_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to token_optimization_results.json")
    print("="*70)
