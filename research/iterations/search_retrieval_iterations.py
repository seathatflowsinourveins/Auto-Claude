"""
SEARCH & RETRIEVAL ITERATIONS - Information Retrieval Patterns
===============================================================
Query processing, ranking, hybrid search, semantic search
"""
...
SEARCH_TOPICS = [
    # Query Processing
    {"topic": "Query understanding: intent classification, entity extraction", "area": "query"},
    {"topic": "Query expansion: synonyms, related terms, HyDE", "area": "query"},
    {"topic": "Query rewriting: LLM-based reformulation, decomposition", "area": "query"},
    {"topic": "Multi-query retrieval: generate multiple search queries", "area": "query"},

    # Dense Retrieval
    {"topic": "Bi-encoder retrieval: query and document encoders", "area": "dense"},
    {"topic": "Approximate nearest neighbor: HNSW, IVF, PQ", "area": "dense"},
    {"topic": "Dense retrieval training: contrastive learning, hard negatives", "area": "dense"},
    {"topic": "Zero-shot dense retrieval: instruction-following embeddings", "area": "dense"},

    # Sparse Retrieval
    {"topic": "BM25: TF-IDF, document length normalization", "area": "sparse"},
    {"topic": "SPLADE: learned sparse representations", "area": "sparse"},
    {"topic": "Keyword extraction: RAKE, YAKE, KeyBERT", "area": "sparse"},
    {"topic": "Inverted index optimization: sharding, caching", "area": "sparse"},

    # Hybrid Search
    {"topic": "Hybrid search fusion: RRF, weighted combination", "area": "hybrid"},
    {"topic": "Two-stage retrieval: recall then rerank", "area": "hybrid"},
    {"topic": "Multi-index search: separate dense and sparse indexes", "area": "hybrid"},
    {"topic": "Adaptive retrieval: dynamic strategy selection", "area": "hybrid"},

    # Reranking
    {"topic": "Cross-encoder reranking: BERT-based relevance scoring", "area": "reranking"},
    {"topic": "LLM reranking: listwise, pairwise, pointwise", "area": "reranking"},
    {"topic": "Cohere Rerank: API-based reranking service", "area": "reranking"},
    {"topic": "Diversity reranking: MMR, result diversification", "area": "reranking"},
]
...
class SearchExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("search", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Search & Retrieval Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Information retrieval implementation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("search", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> SearchResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return SearchResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("SEARCH & RETRIEVAL ITERATIONS")
    print("="*70)
    ...
    for i, topic_data in enumerate(SEARCH_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("SEARCH & RETRIEVAL COMPLETE")
    print("="*70)
    ...
    with open("search_retrieval_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to search_retrieval_results.json")
    print("="*70)
