"""
CONSTITUTIONAL AI ITERATIONS - AI Safety & Alignment
=====================================================
Constitutional AI, RLHF alternatives, harmlessness
"""
...
CONSTITUTIONAL_AI_TOPICS = [
    # Fundamentals
    {"topic": "Constitutional AI: self-critique training", "area": "fundamentals"},
    {"topic": "RLAIF: reinforcement learning AI feedback", "area": "fundamentals"},
    {"topic": "Harmlessness training: safety criteria", "area": "fundamentals"},
    {"topic": "Red teaming: adversarial testing", "area": "fundamentals"},

    # Methods
    {"topic": "Self-improvement: recursive refinement", "area": "methods"},
    {"topic": "Critique-revision: iterative feedback", "area": "methods"},
    {"topic": "Principle-based training: explicit rules", "area": "methods"},
    {"topic": "Debate: adversarial collaboration", "area": "methods"},

    # Safety
    {"topic": "Jailbreak prevention: prompt attacks", "area": "safety"},
    {"topic": "Harmful content filtering: detection", "area": "safety"},
    {"topic": "Refusal training: appropriate decline", "area": "safety"},
    {"topic": "Deception detection: honesty training", "area": "safety"},

    # Research
    {"topic": "Anthropic CAI: original research", "area": "research"},
    {"topic": "Sleeper agents: hidden behaviors", "area": "research"},
    {"topic": "Scalable oversight: supervision methods", "area": "research"},
    {"topic": "AI alignment: value learning", "area": "research"},

    # Applications
    {"topic": "Claude safety: Anthropic approach", "area": "applications"},
    {"topic": "GPT moderation: OpenAI safety", "area": "applications"},
    {"topic": "Open model safety: Llama guardrails", "area": "applications"},
    {"topic": "Enterprise safety: deployment controls", "area": "applications"},
]

@dataclass
class ConstitutionalAIResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class ConstitutionalAIExecutor:
    def __init__(self):
        self.exa = None
        self.qdrant = None
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("constitutional_ai", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Constitutional AI Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Constitutional AI safety: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("constitutional_ai", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ConstitutionalAIResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ConstitutionalAIResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("CONSTITUTIONAL AI ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(CONSTITUTIONAL_AI_TOPICS)}")
    print("="*70)

    executor = ConstitutionalAIExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(CONSTITUTIONAL_AI_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("CONSTITUTIONAL AI COMPLETE")
    print("="*70)
    ...
    with open("constitutional_ai_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to constitutional_ai_results.json")
    print("="*70)
