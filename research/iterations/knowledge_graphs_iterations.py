...
class KGExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("kg", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Knowledge Graphs Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Knowledge graph implementation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("kg", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> KGResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return KGResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("KNOWLEDGE GRAPHS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(KG_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(KG_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("KNOWLEDGE GRAPHS COMPLETE")
    print("="*70)
    ...
    with open("knowledge_graphs_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to knowledge_graphs_results.json")
    print("="*70)
