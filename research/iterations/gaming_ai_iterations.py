"""
GAMING AI ITERATIONS - Game AI & NPCs
=====================================
Game AI, procedural generation, NPCs
"""
...
GAMING_TOPICS = [
    # Game AI
    {"topic": "Behavior trees for NPCs: decision making, AI directors", "area": "game_ai"},
    {"topic": "Finite state machines: enemy AI, game logic", "area": "game_ai"},
    {"topic": "Goal-oriented action planning (GOAP): dynamic AI", "area": "game_ai"},
    {"topic": "Monte Carlo tree search: game playing AI", "area": "game_ai"},

    # LLM NPCs
    {"topic": "LLM-powered NPCs: dialogue, personality, memory", "area": "llm_npc"},
    {"topic": "Character AI: creating believable game characters", "area": "llm_npc"},
    {"topic": "Dynamic dialogue systems: context-aware conversation", "area": "llm_npc"},
    {"topic": "NPC memory systems: remembering player interactions", "area": "llm_npc"},

    # Procedural Generation
    {"topic": "Procedural content generation: levels, maps, items", "area": "procedural"},
    {"topic": "AI dungeon generation: roguelike level design", "area": "procedural"},
    {"topic": "Narrative generation: AI-created stories, quests", "area": "procedural"},
    {"topic": "Texture and asset generation: AI art for games", "area": "procedural"},

    # Game Playing
    {"topic": "Reinforcement learning for games: OpenAI Five, AlphaStar", "area": "playing"},
    {"topic": "Game testing with AI: QA automation, bug finding", "area": "playing"},
    {"topic": "Speedrun AI: optimal game completion strategies", "area": "playing"},
    {"topic": "Game balance with AI: difficulty tuning, analytics", "area": "playing"},

    # Tools
    {"topic": "Unity ML-Agents: training AI in Unity games", "area": "tools"},
    {"topic": "Unreal Engine AI: behavior trees, EQS, NavMesh", "area": "tools"},
    {"topic": "Inworld AI: character engine for games", "area": "tools"},
    {"topic": "Convai: conversational AI for virtual worlds", "area": "tools"},
]
...
class GamingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("gaming", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Gaming AI Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Gaming AI: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("gaming", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> GamingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return GamingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("GAMING AI ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(GAMING_TOPICS)}")
    print("="*70)

    executor = GamingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(GAMING_TOPICS, 1):
        try:
    ...
    with open("gaming_ai_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to gaming_ai_results.json")
    print("="*70)
