"""
ATTENTION MECHANISMS ITERATIONS - Transformers
==============================================
Attention, transformers, position encoding
"""
...
ATTENTION_TOPICS = [
    # Core Attention
    {"topic": "Self-attention: query key value mechanism", "area": "core"},
    {"topic": "Multi-head attention: parallel attention", "area": "core"},
    {"topic": "Cross-attention: encoder-decoder attention", "area": "core"},
    {"topic": "Causal attention: autoregressive masking", "area": "core"},

    # Efficient Attention
    {"topic": "Flash Attention: IO-aware algorithm", "area": "efficient"},
    {"topic": "Linear attention: O(n) complexity", "area": "efficient"},
    {"topic": "Sparse attention: BigBird, Longformer", "area": "efficient"},
    {"topic": "Sliding window attention: local context", "area": "efficient"},

    # Position Encoding
    {"topic": "Sinusoidal positional encoding: original", "area": "position"},
    {"topic": "RoPE: rotary position embedding", "area": "position"},
    {"topic": "ALiBi: attention linear bias", "area": "position"},
    {"topic": "Learned positional embeddings: trainable", "area": "position"},

    # Variants
    {"topic": "Grouped query attention: GQA", "area": "variants"},
    {"topic": "Multi-query attention: MQA", "area": "variants"},
    {"topic": "Differential attention: Microsoft", "area": "variants"},
    {"topic": "Ring attention: distributed context", "area": "variants"},

    # Long Context
    {"topic": "Context extension: extending transformers", "area": "long-context"},
    {"topic": "YaRN: yet another RoPE extension", "area": "long-context"},
    {"topic": "Landmark attention: selective memory", "area": "long-context"},
    {"topic": "Infinite context: streaming attention", "area": "long-context"},
]
...
class AttentionExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("attention_mechanisms", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Attention Mechanisms Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Attention mechanisms: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("attention_mechanisms", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AttentionResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AttentionResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("ATTENTION MECHANISMS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(ATTENTION_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(ATTENTION_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to attention_mechanisms_results.json")
    print("="*70)
