...
class AgentMemoryExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("agent_memory", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Agent Memory Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI agent memory systems: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("agent_memory", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AgentMemoryResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AgentMemoryResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AGENT MEMORY ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AGENT_MEMORY_TOPICS)}")
    print("="*70)

    executor = AgentMemoryExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(AGENT_MEMORY_TOPICS, 1):
        try:
    ...
    with open("agent_memory_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to agent_memory_results.json")
    print("="*70)
