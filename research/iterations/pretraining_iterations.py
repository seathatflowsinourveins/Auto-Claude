"""
PRETRAINING ITERATIONS - Foundation Training
=============================================
Pretraining, data, compute, infrastructure
"""
...
PRETRAINING_TOPICS = [
    # Data
    {"topic": "Pretraining data: web crawls, Common Crawl", "area": "data"},
    {"topic": "Data filtering: quality, deduplication", "area": "data"},
    {"topic": "Data mixing: proportions, curriculum", "area": "data"},
    {"topic": "Synthetic pretraining data: generation", "area": "data"},

    # Training
    {"topic": "Training dynamics: loss curves, checkpoints", "area": "training"},
    {"topic": "Learning rate schedules: warmup, decay", "area": "training"},
    {"topic": "Batch size scaling: large batch training", "area": "training"},
    {"topic": "Training instabilities: spike prevention", "area": "training"},

    # Infrastructure
    {"topic": "Distributed training: FSDP, DeepSpeed", "area": "infrastructure"},
    {"topic": "GPU clusters: A100, H100 training", "area": "infrastructure"},
    {"topic": "Checkpointing: fault tolerance", "area": "infrastructure"},
    {"topic": "Training cost: compute economics", "area": "infrastructure"},

    # Techniques
    {"topic": "Causal language modeling: next token", "area": "techniques"},
    {"topic": "Masked language modeling: BERT-style", "area": "techniques"},
    {"topic": "Contrastive pretraining: CLIP approach", "area": "techniques"},
    {"topic": "Multi-task pretraining: T5 approach", "area": "techniques"},

    # Efficiency
    {"topic": "Mixed precision training: FP16, BF16", "area": "efficiency"},
    {"topic": "Gradient compression: communication", "area": "efficiency"},
    {"topic": "Memory optimization: activation checkpointing", "area": "efficiency"},
    {"topic": "Data loading: efficient pipelines", "area": "efficiency"},
]
...
class PretrainingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("pretraining", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Pretraining Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM pretraining: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("pretraining", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> PretrainingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return PretrainingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("PRETRAINING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(PRETRAINING_TOPICS)}")
    print("="*70)

    executor = PretrainingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(PRETRAINING_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to pretraining_results.json")
    print("="*70)
