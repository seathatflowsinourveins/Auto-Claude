"""
AI TESTING ITERATIONS - ML Testing & QA
========================================
AI testing, ML testing, model QA
"""
...
AI_TESTING_TOPICS = [
    # Frameworks
    {"topic": "Great Expectations: data testing", "area": "frameworks"},
    {"topic": "Deepchecks: ML validation", "area": "frameworks"},
    {"topic": "Evidently: ML monitoring tests", "area": "frameworks"},
    {"topic": "PromptFoo: prompt testing", "area": "frameworks"},

    # Data Testing
    {"topic": "Data validation: schema checks", "area": "data_testing"},
    {"topic": "Data drift: distribution shift", "area": "data_testing"},
    {"topic": "Data quality: completeness", "area": "data_testing"},
    {"topic": "Feature testing: input validation", "area": "data_testing"},

    # Model Testing
    {"topic": "Model validation: accuracy testing", "area": "model_testing"},
    {"topic": "Behavioral testing: CheckList", "area": "model_testing"},
    {"topic": "Adversarial testing: robustness", "area": "model_testing"},
    {"topic": "Regression testing: model changes", "area": "model_testing"},

    # LLM Testing
    {"topic": "Prompt testing: evaluation", "area": "llm_testing"},
    {"topic": "LLM evals: benchmark testing", "area": "llm_testing"},
    {"topic": "Red teaming: safety testing", "area": "llm_testing"},
    {"topic": "Hallucination testing: factuality", "area": "llm_testing"},

    # Production
    {"topic": "Shadow testing: production comparison", "area": "production"},
    {"topic": "Canary deployment: gradual rollout", "area": "production"},
    {"topic": "Continuous testing: CI/CD ML", "area": "production"},
    {"topic": "Performance testing: latency", "area": "production"},
]

@dataclass
class AITestingResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class AITestingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("ai_testing", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] AI Testing Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI ML testing QA: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("ai_testing", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AITestingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AITestingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AI TESTING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AI_TESTING_TOPICS)}")
    print("="*70)

    executor = AITestingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(AI_TESTING_TOPICS, 1):
        try:
    ...
    with open("ai_testing_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to ai_testing_results.json")
    print("="*70)
