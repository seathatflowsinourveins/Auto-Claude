"""
LEGAL AI ITERATIONS - LegalTech & Contract AI
==============================================
Contract analysis, legal research, compliance
"""
...
LEGAL_TOPICS = [
    # Contract Analysis
    {"topic": "Contract review automation: clause extraction, risk flagging", "area": "contracts"},
    {"topic": "Contract comparison: version diff, term analysis", "area": "contracts"},
    {"topic": "Contract generation: templates, customization with AI", "area": "contracts"},
    {"topic": "Due diligence automation: M&A document review", "area": "contracts"},

    # Legal Research
    {"topic": "Legal research with AI: case law search, citation analysis", "area": "research"},
    {"topic": "Legal question answering: statute interpretation", "area": "research"},
    {"topic": "Precedent analysis: similar case identification", "area": "research"},
    {"topic": "Legal document summarization: briefs, opinions", "area": "research"},

    # Legal NLP
    {"topic": "Legal NER: parties, dates, obligations, definitions", "area": "nlp"},
    {"topic": "Legal text classification: document type, jurisdiction", "area": "nlp"},
    {"topic": "Legal language models: LegalBERT, specialized training", "area": "nlp"},
    {"topic": "Multi-jurisdictional analysis: cross-border legal AI", "area": "nlp"},

    # Compliance
    {"topic": "Regulatory compliance: monitoring, change management", "area": "compliance"},
    {"topic": "Privacy compliance: GDPR, CCPA automation", "area": "compliance"},
    {"topic": "E-discovery: document review, relevance classification", "area": "compliance"},
    {"topic": "Audit trail generation: documentation automation", "area": "compliance"},

    # Access to Justice
    {"topic": "Legal chatbots: self-help, intake automation", "area": "access"},
    {"topic": "Plain language: legal document simplification", "area": "access"},
    {"topic": "Pro bono matching: AI for legal aid organizations", "area": "access"},
    {"topic": "Court filing assistance: form completion, validation", "area": "access"},
]
...
class LegalExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("legal", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Legal AI Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Legal AI technology: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("legal", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> LegalResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return LegalResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("LEGAL AI ITERATIONS")
    print("="*70)
    ...
    for i, topic_data in enumerate(LEGAL_TOPICS, 1):
        try:
    ...
    with open("legal_ai_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to legal_ai_results.json")
    print("="*70)
