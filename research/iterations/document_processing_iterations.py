"""
DOCUMENT PROCESSING ITERATIONS - Document AI & Extraction
=========================================================
PDF processing, OCR, document understanding, parsing
"""
...
DOCUMENT_TOPICS = [
    # PDF Processing
    {"topic": "PDF parsing for LLMs: extracting text, tables, images", "area": "pdf"},
    {"topic": "PDF chunking strategies: page-aware, semantic splitting", "area": "pdf"},
    {"topic": "PDF layout understanding: multi-column, headers, footers", "area": "pdf"},
    {"topic": "Unstructured.io: comprehensive document parsing", "area": "pdf"},

    # OCR & Vision
    {"topic": "OCR for documents: Tesseract, cloud APIs, accuracy", "area": "ocr"},
    {"topic": "Document image preprocessing: deskew, binarization", "area": "ocr"},
    {"topic": "Handwriting recognition: extracting handwritten text", "area": "ocr"},
    {"topic": "Table extraction from images: structure recognition", "area": "ocr"},

    # Document Understanding
    {"topic": "Document classification: categorizing document types", "area": "understanding"},
    {"topic": "Key-value extraction: invoices, receipts, forms", "area": "understanding"},
    {"topic": "Document QA: answering questions about documents", "area": "understanding"},
    {"topic": "Multi-document reasoning: synthesizing across files", "area": "understanding"},

    # Tools & Services
    {"topic": "LlamaParse: multimodal document parsing", "area": "tools"},
    {"topic": "Azure Document Intelligence: enterprise parsing", "area": "tools"},
    {"topic": "AWS Textract: document analysis at scale", "area": "tools"},
    {"topic": "Google Document AI: form parsing, entity extraction", "area": "tools"},

    # Pipelines
    {"topic": "Document RAG pipelines: ingestion to retrieval", "area": "pipelines"},
    {"topic": "Document preprocessing: cleaning, normalization", "area": "pipelines"},
    {"topic": "Metadata extraction: dates, authors, titles", "area": "pipelines"},
    {"topic": "Document versioning: tracking changes over time", "area": "pipelines"},
]
...
class DocumentExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("document", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Document Processing Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Document processing AI: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("document", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> DocumentResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return DocumentResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("DOCUMENT PROCESSING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(DOCUMENT_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(DOCUMENT_TOPICS, 1):
        try:
    ...
    with open("document_processing_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to document_processing_results.json")
    print("="*70)
