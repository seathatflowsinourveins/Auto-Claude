"""
LLM SERVING ITERATIONS - Inference Infrastructure
=================================================
Serving, inference, vLLM, TGI, optimization
"""
...
LLM_SERVING_TOPICS = [
    # Inference Engines
    {"topic": "vLLM: PagedAttention, fast serving", "area": "engines"},
    {"topic": "TGI: Text Generation Inference, HuggingFace", "area": "engines"},
    {"topic": "TensorRT-LLM: NVIDIA inference", "area": "engines"},
    {"topic": "llama.cpp: CPU inference, GGUF", "area": "engines"},

    # Optimization
    {"topic": "KV cache: memory optimization", "area": "optimization"},
    {"topic": "Continuous batching: dynamic batching", "area": "optimization"},
    {"topic": "Speculative decoding: draft models", "area": "optimization"},
    {"topic": "Prefix caching: prompt reuse", "area": "optimization"},

    # Scaling
    {"topic": "Load balancing: request distribution", "area": "scaling"},
    {"topic": "Auto-scaling: demand-based scaling", "area": "scaling"},
    {"topic": "Multi-GPU serving: tensor parallel", "area": "scaling"},
    {"topic": "Serverless LLM: on-demand inference", "area": "scaling"},

    # Platforms
    {"topic": "Replicate: model hosting platform", "area": "platforms"},
    {"topic": "Together AI: inference platform", "area": "platforms"},
    {"topic": "Groq: LPU inference hardware", "area": "platforms"},
    {"topic": "Modal: serverless GPU compute", "area": "platforms"},

    # Metrics
    {"topic": "Latency optimization: time-to-first-token", "area": "metrics"},
    {"topic": "Throughput: tokens per second", "area": "metrics"},
    {"topic": "Cost efficiency: price per token", "area": "metrics"},
    {"topic": "Quality vs speed: tradeoffs", "area": "metrics"},
]
...
class LLMServingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("llm_serving", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] LLM Serving Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM serving: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("llm_serving", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> LLMServingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return LLMServingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("LLM SERVING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(LLM_SERVING_TOPICS)}")
    print("="*70)

    executor = LLMServingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(LLM_SERVING_TOPICS, 1):
        try:
    ...
    with open("llm_serving_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to llm_serving_results.json")
    print("="*70)
