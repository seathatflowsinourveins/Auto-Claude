"""
SEARCH & DISCOVERY AI ITERATIONS - Information Retrieval
=========================================================
Search engines, semantic search, discovery, ranking
"""
...
SEARCH_DISCOVERY_TOPICS = [
    # Semantic Search
    {"topic": "Semantic search: embedding-based document retrieval", "area": "semantic"},
    {"topic": "Hybrid search: combining keyword and vector search", "area": "semantic"},
    {"topic": "Cross-encoder reranking: improving search relevance", "area": "semantic"},
    {"topic": "Multilingual search: cross-lingual information retrieval", "area": "semantic"},

    # Search Infrastructure
    {"topic": "Elasticsearch with AI: neural search, ELSER", "area": "infrastructure"},
    {"topic": "Apache Solr ML: learning to rank, NLP", "area": "infrastructure"},
    {"topic": "Meilisearch: instant search, typo tolerance", "area": "infrastructure"},
    {"topic": "Typesense: fast, typo-tolerant search engine", "area": "infrastructure"},

    # Ranking & Personalization
    {"topic": "Learning to rank: ML-based search ranking", "area": "ranking"},
    {"topic": "Personalized search: user context, preferences", "area": "ranking"},
    {"topic": "Query understanding: intent, expansion, rewriting", "area": "ranking"},
    {"topic": "Click models: implicit feedback for ranking", "area": "ranking"},

    # Discovery & Recommendation
    {"topic": "Content discovery: serendipitous recommendations", "area": "discovery"},
    {"topic": "Similar item search: find-alike, visual search", "area": "discovery"},
    {"topic": "Faceted search: dynamic filtering, navigation", "area": "discovery"},
    {"topic": "Autocomplete: query suggestions, completion", "area": "discovery"},

    # Platforms
    {"topic": "Algolia: AI-powered search and discovery", "area": "platforms"},
    {"topic": "Pinecone: vector database for AI search", "area": "platforms"},
    {"topic": "Weaviate: vector search engine with modules", "area": "platforms"},
    {"topic": "Vespa: big data serving engine, search", "area": "platforms"},
]
...
class SearchDiscoveryExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("search_discovery", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Search & Discovery Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI-powered search and discovery: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("search_discovery", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> SearchDiscoveryResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return SearchDiscoveryResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("SEARCH & DISCOVERY AI ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(SEARCH_DISCOVERY_TOPICS)}")
    print("="*70)

    executor = SearchDiscoveryExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(SEARCH_DISCOVERY_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("SEARCH & DISCOVERY AI COMPLETE")
    print("="*70)
    ...
    with open("search_discovery_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to search_discovery_results.json")
    print("="*70)
