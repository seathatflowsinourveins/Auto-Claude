"""
GRAPH ML ITERATIONS - Graph Neural Networks
============================================
GNN, graph learning, networks, embeddings
"""
...
GRAPH_ML_TOPICS = [
    # GNN Architectures
    {"topic": "Graph neural networks: GCN, GAT, GraphSAGE", "area": "architectures"},
    {"topic": "Message passing neural networks: MPNN framework", "area": "architectures"},
    {"topic": "Graph transformers: attention on graphs", "area": "architectures"},
    {"topic": "Heterogeneous graphs: multi-type nodes, edges", "area": "architectures"},

    # Learning Tasks
    {"topic": "Node classification: predicting node labels", "area": "tasks"},
    {"topic": "Link prediction: predicting connections", "area": "tasks"},
    {"topic": "Graph classification: whole graph labels", "area": "tasks"},
    {"topic": "Graph generation: creating new graphs", "area": "tasks"},

    # Applications
    {"topic": "Molecular GNN: drug discovery, chemistry", "area": "applications"},
    {"topic": "Social network analysis: community detection", "area": "applications"},
    {"topic": "Knowledge graph embeddings: TransE, RotatE", "area": "applications"},
    {"topic": "Recommendation graphs: user-item networks", "area": "applications"},

    # Techniques
    {"topic": "Graph self-supervised learning: contrastive", "area": "techniques"},
    {"topic": "Scalable GNN: mini-batch, sampling", "area": "techniques"},
    {"topic": "Dynamic graphs: temporal GNN", "area": "techniques"},
    {"topic": "Explainable GNN: interpretation", "area": "techniques"},

    # Tools
    {"topic": "PyG: PyTorch Geometric library", "area": "tools"},
    {"topic": "DGL: Deep Graph Library", "area": "tools"},
    {"topic": "NetworkX: graph analysis Python", "area": "tools"},
    {"topic": "Neo4j: graph database + ML", "area": "tools"},
]
...
class GraphMLExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("graph_ml", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Graph ML Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Graph machine learning: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("graph_ml", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> GraphMLResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return GraphMLResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("GRAPH ML ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(GRAPH_ML_TOPICS)}")
    print("="*70)

    executor = GraphMLExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(GRAPH_ML_TOPICS, 1):
        try:
    ...
    with open("graph_ml_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to graph_ml_results.json")
    print("="*70)
