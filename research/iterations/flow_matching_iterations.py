"""
FLOW MATCHING ITERATIONS - Next-Gen Generative
===============================================
...
FLOW_MATCHING_TOPICS = [
    # Fundamentals
    {"topic": "Flow matching: continuous normalizing flows", "area": "fundamentals"},
    {"topic": "Rectified flow: straight paths", "area": "fundamentals"},
    {"topic": "Optimal transport: Wasserstein distance", "area": "fundamentals"},
    {"topic": "Probability flow ODE: score-based", "area": "fundamentals"},

    # Models
    {"topic": "Stable Diffusion 3: flow matching", "area": "models"},
    {"topic": "FLUX: Black Forest Labs model", "area": "models"},
    {"topic": "SD3 architecture: MMDiT blocks", "area": "models"},
    {"topic": "Video flow matching: temporal generation", "area": "models"},

    # Techniques
    {"topic": "Conditional flow matching: CFM", "area": "techniques"},
    {"topic": "Optimal transport paths: OT-CFM", "area": "techniques"},
    {"topic": "Variance preserving: VP flow", "area": "techniques"},
    {"topic": "Mini-batch OT: scalable training", "area": "techniques"},

    # Advantages
    {"topic": "Flow vs diffusion: comparison", "area": "advantages"},
    {"topic": "Simulation-free training: efficiency", "area": "advantages"},
    {"topic": "Straighter paths: fewer steps", "area": "advantages"},
    {"topic": "Better interpolation: smoother generation", "area": "advantages"},

    # Applications
    {"topic": "Image generation flow: high quality", "area": "applications"},
    {"topic": "Audio flow: music, speech", "area": "applications"},
    {"topic": "Molecule generation: drug discovery", "area": "applications"},
    {"topic": "3D flow: shape generation", "area": "applications"},
]
...
class FlowMatchingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("flow_matching", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Flow Matching Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Flow matching generative: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("flow_matching", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> FlowMatchingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return FlowMatchingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("FLOW MATCHING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(FLOW_MATCHING_TOPICS)}")
    print("="*70)

    executor = FlowMatchingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(FLOW_MATCHING_TOPICS, 1):
        try:
    ...
    with open("flow_matching_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to flow_matching_results.json")
    print("="*70)
