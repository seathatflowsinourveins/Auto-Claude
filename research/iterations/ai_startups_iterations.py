"""
AI STARTUPS ITERATIONS - Emerging Companies
============================================
AI startups, funding, products, trends
"""
...
AI_STARTUPS_TOPICS = [
    ...
    {"topic": "Anthropic: Claude AI, constitutional AI", "area": "foundation"},
    {"topic": "Mistral AI: European LLM company, open weights", "area": "foundation"},
    {"topic": "Cohere: enterprise LLM, embeddings", "area": "foundation"},
    {"topic": "AI21 Labs: Jurassic models, Wordtune", "area": "foundation"},

    # AI Infrastructure
    {"topic": "Anyscale: Ray, distributed computing startup", "area": "infrastructure"},
    {"topic": "Modal: serverless GPU compute platform", "area": "infrastructure"},
    {"topic": "Weights & Biases: ML experiment tracking", "area": "infrastructure"},
    {"topic": "Scale AI: data labeling, annotation", "area": "infrastructure"},

    # AI Applications
    {"topic": "Jasper AI: marketing content generation", "area": "applications"},
    {"topic": "Copy.ai: AI copywriting platform", "area": "applications"},
    {"topic": "Harvey: AI for legal professionals", "area": "applications"},
    {"topic": "Glean: enterprise AI search", "area": "applications"},

    # AI Coding
    {"topic": "Replit: AI-powered IDE, Ghostwriter", "area": "coding"},
    {"topic": "Codeium: free AI code completion", "area": "coding"},
    {"topic": "Poolside AI: code generation startup", "area": "coding"},
    {"topic": "Magic AI: long-context code models", "area": "coding"},

    # AI Research
    {"topic": "xAI: Elon Musk AI company, Grok", "area": "research"},
    {"topic": "Inflection AI: Pi personal AI", "area": "research"},
    {"topic": "Adept AI: action models, ACT-1", "area": "research"},
    {"topic": "Character AI: conversational AI characters", "area": "research"},
]

@dataclass
class AIStartupsResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class AIStartupsExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("ai_startups", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] AI Startups Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI startup company: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("ai_startups", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AIStartupsResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AIStartupsResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AI STARTUPS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AI_STARTUPS_TOPICS)}")
    print("="*70)

    executor = AIStartupsExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(AI_STARTUPS_TOPICS, 1):
        try:
    ...
    with open("ai_startups_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to ai_startups_results.json")
    print("="*70)
