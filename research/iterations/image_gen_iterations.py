"""
IMAGE GEN ITERATIONS - Image Generation Models
==============================================
Image generation, text-to-image, image AI
"""
...
IMAGE_GEN_TOPICS = [
    # Models
    {"topic": "DALL-E 3: OpenAI image generation", "area": "models"},
    {"topic": "Midjourney v6: artistic generation", "area": "models"},
    {"topic": "Stable Diffusion XL: open source", "area": "models"},
    {"topic": "Imagen 3: Google image AI", "area": "models"},

    # Architectures
    {"topic": "Latent diffusion: compressed generation", "area": "architectures"},
    {"topic": "DiT: diffusion transformer", "area": "architectures"},
    {"topic": "Autoencoder: VAE image", "area": "architectures"},
    {"topic": "GAN: generative adversarial", "area": "architectures"},

    # Control
    {"topic": "ControlNet: spatial conditioning", "area": "control"},
    {"topic": "IP-Adapter: image prompting", "area": "control"},
    {"topic": "Inpainting: region editing", "area": "control"},
    {"topic": "Outpainting: image extension", "area": "control"},

    # Optimization
    {"topic": "LCM: latent consistency", "area": "optimization"},
    {"topic": "SDXL Turbo: fast generation", "area": "optimization"},
    {"topic": "Flash diffusion: accelerated", "area": "optimization"},
    {"topic": "Quantized diffusion: efficient", "area": "optimization"},

    # Applications
    {"topic": "AI art: creative generation", "area": "applications"},
    {"topic": "Product photography: e-commerce", "area": "applications"},
    {"topic": "Design AI: graphic design", "area": "applications"},
    {"topic": "Avatar generation: profile images", "area": "applications"},
]
...
class ImageGenExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("image_gen", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Image Gen Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI image generation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("image_gen", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ImageGenResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ImageGenResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("IMAGE GEN ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(IMAGE_GEN_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(IMAGE_GEN_TOPICS, 1):
        try:
    ...
    with open("image_gen_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to image_gen_results.json")
    print("="*70)
