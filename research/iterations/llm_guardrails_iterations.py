"""
LLM GUARDRAILS ITERATIONS - Safety & Control
=============================================
LLM guardrails, safety, content moderation
"""
...
LLM_GUARDRAILS_TOPICS = [
    # Frameworks
    {"topic": "NeMo Guardrails: NVIDIA safety", "area": "frameworks"},
    {"topic": "Guardrails AI: validation framework", "area": "frameworks"},
    {"topic": "LLM Guard: content safety", "area": "frameworks"},
    {"topic": "Rebuff: prompt injection defense", "area": "frameworks"},

    # Input Safety
    {"topic": "Prompt injection: attack prevention", "area": "input_safety"},
    {"topic": "Jailbreak detection: boundary testing", "area": "input_safety"},
    {"topic": "Input validation: sanitization", "area": "input_safety"},
    {"topic": "PII detection: privacy protection", "area": "input_safety"},

    # Output Safety
    {"topic": "Content moderation: harmful output", "area": "output_safety"},
    {"topic": "Hallucination detection: factuality", "area": "output_safety"},
    {"topic": "Toxicity filtering: safe responses", "area": "output_safety"},
    {"topic": "Bias mitigation: fair output", "area": "output_safety"},

    # Control
    {"topic": "Output validation: schema check", "area": "control"},
    {"topic": "Rate limiting: abuse prevention", "area": "control"},
    {"topic": "Topic guardrails: conversation bounds", "area": "control"},
    {"topic": "Action guardrails: tool safety", "area": "control"},

    # Production
    {"topic": "Guardrails monitoring: observability", "area": "production"},
    {"topic": "Guardrails testing: red teaming", "area": "production"},
    {"topic": "Guardrails MLOps: deployment", "area": "production"},
    {"topic": "Enterprise guardrails: compliance", "area": "production"},
]

@dataclass
class LLMGuardrailsResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class LLMGuardrailsExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("llm_guardrails", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] LLM Guardrails Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM guardrails safety: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("llm_guardrails", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> LLMGuardrailsResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return LLMGuardrailsResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("LLM GUARDRAILS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(LLM_GUARDRAILS_TOPICS)}")
    print("="*70)

    executor = LLMGuardrailsExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(LLM_GUARDRAILS_TOPICS, 1):
        try:
    ...
    with open("llm_guardrails_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to llm_guardrails_results.json")
    print("="*70)
