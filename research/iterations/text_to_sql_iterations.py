"""
TEXT TO SQL ITERATIONS - Natural Language to SQL
================================================
Text-to-SQL, NL2SQL, database querying
"""
...
TEXT_TO_SQL_TOPICS = [
    # Models
    {"topic": "Text-to-SQL: natural language queries", "area": "models"},
    {"topic": "DIN-SQL: decomposed SQL generation", "area": "models"},
    {"topic": "DAIL-SQL: demonstration enhanced", "area": "models"},
    {"topic": "Vanna AI: text-to-SQL platform", "area": "models"},

    # Techniques
    {"topic": "Schema linking: table column matching", "area": "techniques"},
    {"topic": "SQL chain-of-thought: step reasoning", "area": "techniques"},
    {"topic": "Self-correction: SQL validation", "area": "techniques"},
    {"topic": "Few-shot SQL: example-based", "area": "techniques"},

    # Benchmarks
    {"topic": "Spider: text-to-SQL benchmark", "area": "benchmarks"},
    {"topic": "WikiSQL: simple queries", "area": "benchmarks"},
    {"topic": "BIRD: real-world SQL", "area": "benchmarks"},
    {"topic": "SParC: context-dependent", "area": "benchmarks"},

    # Tools
    {"topic": "LangChain SQL: database agents", "area": "tools"},
    {"topic": "SQLCoder: code-focused model", "area": "tools"},
    {"topic": "DuckDB AI: analytical SQL", "area": "tools"},
    {"topic": "Defog: enterprise text-to-SQL", "area": "tools"},

    # Production
    {"topic": "SQL injection: security concerns", "area": "production"},
    {"topic": "Query optimization: performance", "area": "production"},
    {"topic": "Semantic layer: business logic", "area": "production"},
    {"topic": "SQL caching: repeated queries", "area": "production"},
]
...
class TextToSQLExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("text_to_sql", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Text-to-SQL Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Text-to-SQL NL2SQL: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("text_to_sql", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> TextToSQLResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return TextToSQLResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("TEXT TO SQL ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(TEXT_TO_SQL_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(TEXT_TO_SQL_TOPICS, 1):
        try:
    ...
    with open("text_to_sql_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to text_to_sql_results.json")
    print("="*70)
