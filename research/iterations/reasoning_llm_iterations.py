...
class ReasoningLLMExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("reasoning_llm", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Reasoning LLM Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM reasoning: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("reasoning_llm", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ReasoningLLMResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ReasoningLLMResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("REASONING LLM ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(REASONING_LLM_TOPICS)}")
    print("="*70)

    executor = ReasoningLLMExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(REASONING_LLM_TOPICS, 1):
        try:
    ...
    with open("reasoning_llm_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to reasoning_llm_results.json")
    print("="*70)
