"""
IN-CONTEXT LEARNING ITERATIONS - Prompt Techniques
===================================================
...
ICL_PROMPTING_TOPICS = [
    # Fundamentals
    {"topic": "In-context learning: few-shot prompting", "area": "fundamentals"},
    {"topic": "Zero-shot prompting: no examples", "area": "fundamentals"},
    {"topic": "Chain-of-thought: step reasoning", "area": "fundamentals"},
    {"topic": "System prompts: persona and context", "area": "fundamentals"},

    # Techniques
    {"topic": "Tree of thoughts: branching reasoning", "area": "techniques"},
    {"topic": "Self-consistency: multiple paths", "area": "techniques"},
    {"topic": "ReAct: reasoning and acting", "area": "techniques"},
    {"topic": "Least-to-most: decomposition", "area": "techniques"},

    # Advanced
    {"topic": "Meta-prompting: prompt optimization", "area": "advanced"},
    {"topic": "Automatic prompt engineering: APE", "area": "advanced"},
    {"topic": "DSPy: programmatic prompts", "area": "advanced"},
    {"topic": "Prompt injection: security risks", "area": "advanced"},

    # Structured
    {"topic": "JSON mode: structured output", "area": "structured"},
    {"topic": "Function calling: tool use prompts", "area": "structured"},
    {"topic": "Schema enforcement: type safety", "area": "structured"},
    {"topic": "Constrained generation: grammar", "area": "structured"},

    # Applications
    {"topic": "RAG prompting: retrieval context", "area": "applications"},
    {"topic": "Agent prompting: tool orchestration", "area": "applications"},
    {"topic": "Code generation prompts: programming", "area": "applications"},
    {"topic": "Multimodal prompts: vision-language", "area": "applications"},
]

@dataclass
class ICLPromptingResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class ICLPromptingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("icl_prompting", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] ICL Prompting Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"In-context learning prompting: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("icl_prompting", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ICLPromptingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ICLPromptingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("IN-CONTEXT LEARNING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(ICL_PROMPTING_TOPICS)}")
    print("="*70)

    executor = ICLPromptingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(ICL_PROMPTING_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("IN-CONTEXT LEARNING COMPLETE")
    print("="*70)
    ...
    with open("icl_prompting_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to icl_prompting_results.json")
    print("="*70)
