"""
AI AGENTS & TOOLS ITERATIONS - Tool Use
=======================================
Agents, tool use, function calling, MCP
"""
...
AI_AGENTS_TOOLS_TOPICS = [
    # Tool Use
    {"topic": "LLM tool use: function calling, actions", "area": "tools"},
    {"topic": "ReAct pattern: reasoning and acting", "area": "tools"},
    {"topic": "Tool selection: choosing right tool", "area": "tools"},
    {"topic": "Tool composition: chaining tools", "area": "tools"},

    # Agent Frameworks
    {"topic": "LangChain agents: modular agentic AI", "area": "frameworks"},
    {"topic": "AutoGPT: autonomous task agents", "area": "frameworks"},
    {"topic": "CrewAI: multi-agent collaboration", "area": "frameworks"},
    {"topic": "OpenAI Assistants: stateful agents", "area": "frameworks"},

    # Protocols
    {"topic": "MCP: Model Context Protocol, Anthropic", "area": "protocols"},
    {"topic": "OpenAPI: API-based tool definitions", "area": "protocols"},
    {"topic": "Function schemas: JSON schema tools", "area": "protocols"},
    {"topic": "Tool protocols: standardization", "area": "protocols"},

    # Capabilities
    {"topic": "Code execution: sandboxed environments", "area": "capabilities"},
    {"topic": "Web browsing: agent internet access", "area": "capabilities"},
    {"topic": "File operations: document processing", "area": "capabilities"},
    {"topic": "API calling: external service integration", "area": "capabilities"},

    # Multi-Agent
    {"topic": "Multi-agent systems: collaboration", "area": "multi-agent"},
    {"topic": "Agent orchestration: coordination", "area": "multi-agent"},
    {"topic": "Agent communication: message passing", "area": "multi-agent"},
    {"topic": "Swarm intelligence: collective behavior", "area": "multi-agent"},
]

@dataclass
class AIAgentsToolsResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class AIAgentsToolsExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("ai_agents_tools", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] AI Agents & Tools Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI agents and tools: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("ai_agents_tools", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AIAgentsToolsResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AIAgentsToolsResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AI AGENTS & TOOLS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AI_AGENTS_TOOLS_TOPICS)}")
    print("="*70)

    executor = AIAgentsToolsExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(AI_AGENTS_TOOLS_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("AI AGENTS & TOOLS COMPLETE")
    print("="*70)
    ...
    with open("ai_agents_tools_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to ai_agents_tools_results.json")
    print("="*70)
