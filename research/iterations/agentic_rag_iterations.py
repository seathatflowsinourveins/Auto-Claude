...
class AgenticRAGExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("agentic_rag", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Agentic RAG Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Agentic RAG retrieval: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("agentic_rag", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AgenticRAGResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AgenticRAGResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AGENTIC RAG ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AGENTIC_RAG_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(AGENTIC_RAG_TOPICS, 1):
        try:
    ...
    with open("agentic_rag_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to agentic_rag_results.json")
    print("="*70)
