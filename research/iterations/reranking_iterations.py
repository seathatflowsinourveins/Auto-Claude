...
class RerankingExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("reranking", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Reranking Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Reranking retrieval models: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("reranking", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> RerankingResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return RerankingResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("RERANKING ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(RERANKING_TOPICS)}")
    print("="*70)

    executor = RerankingExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(RERANKING_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to reranking_results.json")
    print("="*70)
