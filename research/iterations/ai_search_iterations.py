"""
AI SEARCH ITERATIONS - Semantic Search & Discovery
==================================================
AI search, semantic search, neural IR
"""
...
AI_SEARCH_TOPICS = [
    # Platforms
    {"topic": "Perplexity AI: answer engine", "area": "platforms"},
    {"topic": "You.com: AI search engine", "area": "platforms"},
    {"topic": "Exa: neural search API", "area": "platforms"},
    {"topic": "Tavily: AI research assistant", "area": "platforms"},

    # Techniques
    {"topic": "Semantic search: meaning-based", "area": "techniques"},
    {"topic": "Hybrid search: keyword + semantic", "area": "techniques"},
    {"topic": "Dense retrieval: neural embeddings", "area": "techniques"},
    {"topic": "Sparse retrieval: BM25 SPLADE", "area": "techniques"},

    # Neural IR
    {"topic": "Neural information retrieval: deep learning", "area": "neural_ir"},
    {"topic": "ColBERT: contextualized embeddings", "area": "neural_ir"},
    {"topic": "SPLADE: sparse lexical matching", "area": "neural_ir"},
    {"topic": "DPR: dense passage retrieval", "area": "neural_ir"},

    # RAG Search
    {"topic": "RAG search: retrieval generation", "area": "rag_search"},
    {"topic": "Query understanding: intent parsing", "area": "rag_search"},
    {"topic": "Multi-hop search: complex queries", "area": "rag_search"},
    {"topic": "Conversational search: dialogue", "area": "rag_search"},

    # Enterprise
    {"topic": "Enterprise search: workplace AI", "area": "enterprise"},
    {"topic": "Elasticsearch AI: vector search", "area": "enterprise"},
    {"topic": "Azure AI Search: cognitive", "area": "enterprise"},
    {"topic": "Google Vertex Search: enterprise", "area": "enterprise"},
]
...
class AISearchExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("ai_search", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] AI Search Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI semantic search: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("ai_search", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AISearchResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AISearchResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("AI SEARCH ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(AI_SEARCH_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(AI_SEARCH_TOPICS, 1):
        try:
    ...
    with open("ai_search_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to ai_search_results.json")
    print("="*70)
