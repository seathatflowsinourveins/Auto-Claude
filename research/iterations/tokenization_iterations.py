"""
TOKENIZATION ITERATIONS - Text Processing
==========================================
Tokenizers, BPE, SentencePiece, vocabulary
"""
...
TOKENIZATION_TOPICS = [
    # Methods
    {"topic": "BPE: byte pair encoding tokenization", "area": "methods"},
    {"topic": "SentencePiece: unsupervised tokenization", "area": "methods"},
    {"topic": "WordPiece: BERT tokenization", "area": "methods"},
    {"topic": "Unigram: probabilistic tokenization", "area": "methods"},

    # Vocabulary
    {"topic": "Vocabulary size: tradeoffs", "area": "vocabulary"},
    {"topic": "Subword tokenization: handling OOV", "area": "vocabulary"},
    {"topic": "Multilingual tokenization: cross-lingual", "area": "vocabulary"},
    {"topic": "Token merging: vocabulary compression", "area": "vocabulary"},

    # Implementations
    {"topic": "HuggingFace tokenizers: fast Rust", "area": "implementations"},
    {"topic": "tiktoken: OpenAI tokenizer", "area": "implementations"},
    {"topic": "Gemma tokenizer: Google approach", "area": "implementations"},
    {"topic": "Llama tokenizer: Meta approach", "area": "implementations"},

    # Challenges
    {"topic": "Tokenization artifacts: boundary issues", "area": "challenges"},
    {"topic": "Numerical tokenization: math tokens", "area": "challenges"},
    {"topic": "Code tokenization: programming languages", "area": "challenges"},
    {"topic": "Special tokens: system, user, assistant", "area": "challenges"},

    # Optimization
    {"topic": "Token efficiency: tokens per character", "area": "optimization"},
    {"topic": "Tokenizer training: corpus selection", "area": "optimization"},
    {"topic": "Dynamic vocabulary: adaptive tokenization", "area": "optimization"},
    {"topic": "Tokenizer-free: byte-level models", "area": "optimization"},
]
...
class TokenizationExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("tokenization", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Tokenization Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Tokenization: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("tokenization", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> TokenizationResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return TokenizationResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("TOKENIZATION ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(TOKENIZATION_TOPICS)}")
    print("="*70)

    executor = TokenizationExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(TOKENIZATION_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to tokenization_results.json")
    print("="*70)
