"""
DIFFUSION MODELS ITERATIONS - Generative AI
============================================
Diffusion, score matching, image generation
"""
...
DIFFUSION_TOPICS = [
    # Fundamentals
    {"topic": "Diffusion models: denoising score matching", "area": "fundamentals"},
    {"topic": "DDPM: denoising diffusion probabilistic", "area": "fundamentals"},
    {"topic": "Score-based models: Langevin dynamics", "area": "fundamentals"},
    {"topic": "Noise schedules: linear, cosine, learned", "area": "fundamentals"},

    # Architectures
    {"topic": "U-Net diffusion: encoder-decoder", "area": "architectures"},
    {"topic": "DiT: diffusion transformers", "area": "architectures"},
    {"topic": "Latent diffusion: compressed space", "area": "architectures"},
    {"topic": "Cascaded diffusion: multi-resolution", "area": "architectures"},

    # Conditioning
    {"topic": "Text-to-image: CLIP conditioning", "area": "conditioning"},
    {"topic": "Classifier-free guidance: CFG scale", "area": "conditioning"},
    {"topic": "ControlNet: spatial conditioning", "area": "conditioning"},
    {"topic": "IP-Adapter: image prompting", "area": "conditioning"},

    # Efficiency
    {"topic": "DDIM: deterministic sampling", "area": "efficiency"},
    {"topic": "Consistency models: one-step generation", "area": "efficiency"},
    {"topic": "Distilled diffusion: fewer steps", "area": "efficiency"},
    {"topic": "LCM: latent consistency models", "area": "efficiency"},

    # Applications
    {"topic": "Stable Diffusion: open image generation", "area": "applications"},
    {"topic": "DALL-E 3: OpenAI image model", "area": "applications"},
    {"topic": "Midjourney: artistic generation", "area": "applications"},
    {"topic": "Video diffusion: Sora, Runway", "area": "applications"},
]
...
class DiffusionExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("diffusion_models", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Diffusion Models Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Diffusion models: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("diffusion_models", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> DiffusionResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return DiffusionResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("DIFFUSION MODELS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(DIFFUSION_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(DIFFUSION_TOPICS, 1):
        try:
    ...
    print("\n" + "="*70)
    print("DIFFUSION MODELS COMPLETE")
    print("="*70)
    ...
    with open("diffusion_models_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to diffusion_models_results.json")
    print("="*70)
