...
class RLHFAlignmentExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("rlhf_alignment", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] RLHF Alignment Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"RLHF and AI alignment: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("rlhf_alignment", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> RLHFAlignmentResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return RLHFAlignmentResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("RLHF & ALIGNMENT ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(RLHF_ALIGNMENT_TOPICS)}")
    print("="*70)

    executor = RLHFAlignmentExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(RLHF_ALIGNMENT_TOPICS, 1):
        try:
    ...
    with open("rlhf_alignment_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to rlhf_alignment_results.json")
    print("="*70)
