"""
ANOMALY DETECTION ITERATIONS - AI Security & Fraud
===================================================
Outlier detection, fraud detection, security monitoring
"""
...
ANOMALY_TOPICS = [
    # Embedding Anomalies
    {"topic": "Embedding-based anomaly detection: semantic outliers", "area": "embeddings"},
    {"topic": "Clustering for anomalies: DBSCAN, isolation forest", "area": "embeddings"},
    {"topic": "Autoencoder anomaly detection: reconstruction error", "area": "embeddings"},
    {"topic": "Contrastive learning for anomalies: self-supervised", "area": "embeddings"},

    # LLM Anomalies
    {"topic": "LLM for log analysis: detecting unusual patterns", "area": "llm"},
    {"topic": "Text anomaly detection: unusual content identification", "area": "llm"},
    {"topic": "Semantic drift detection: content changes over time", "area": "llm"},
    {"topic": "LLM-based root cause analysis: explaining anomalies", "area": "llm"},

    # Security
    {"topic": "AI for threat detection: network security patterns", "area": "security"},
    {"topic": "User behavior analytics: detecting compromised accounts", "area": "security"},
    {"topic": "Phishing detection with AI: email and URL analysis", "area": "security"},
    {"topic": "Malware classification: behavioral analysis with ML", "area": "security"},

    # Fraud Detection
    {"topic": "Transaction fraud detection: real-time scoring", "area": "fraud"},
    {"topic": "Account takeover detection: session anomalies", "area": "fraud"},
    {"topic": "Synthetic identity detection: fake profile identification", "area": "fraud"},
    {"topic": "Click fraud detection: bot vs human patterns", "area": "fraud"},

    # Operations
    {"topic": "AIOps anomaly detection: infrastructure monitoring", "area": "operations"},
    {"topic": "Predictive maintenance: equipment failure prediction", "area": "operations"},
    {"topic": "Time series anomaly detection: Prophet, ARIMA + AI", "area": "operations"},
    {"topic": "Alert fatigue reduction: smart anomaly prioritization", "area": "operations"},
]
...
class AnomalyExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("anomaly", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Anomaly Detection Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI anomaly detection: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("anomaly", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> AnomalyResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return AnomalyResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("ANOMALY DETECTION ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(ANOMALY_TOPICS)}")
    print("="*70)

    executor = AnomalyExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(ANOMALY_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to anomaly_detection_results.json")
    print("="*70)
