...
class MCPExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("mcp", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] MCP Protocol Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Model Context Protocol (MCP): {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("mcp", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> MCPResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return MCPResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("MCP PROTOCOL ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(MCP_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(MCP_TOPICS, 1):
        try:
    ...
    with open("mcp_protocol_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to mcp_protocol_results.json")
    print("="*70)
