...
class LLMAgentsExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("llm_agents", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] LLM Agents Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"LLM autonomous agents: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("llm_agents", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> LLMAgentsResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return LLMAgentsResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("LLM AGENTS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(LLM_AGENTS_TOPICS)}")
    print("="*70)

    executor = LLMAgentsExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(LLM_AGENTS_TOPICS, 1):
        try:
    ...
    with open("llm_agents_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to llm_agents_results.json")
    print("="*70)
