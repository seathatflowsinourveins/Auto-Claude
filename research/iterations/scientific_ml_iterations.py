"""
SCIENTIFIC ML ITERATIONS - Science & Research
==============================================
Physics-informed ML, simulation, discovery
"""
...
SCIENTIFIC_ML_TOPICS = [
    # Physics-Informed
    {"topic": "Physics-informed neural networks: PINNs", "area": "physics"},
    {"topic": "Neural operators: Fourier neural operator", "area": "physics"},
    {"topic": "Differentiable simulation: end-to-end learning", "area": "physics"},
    {"topic": "Symmetry in ML: equivariant neural networks", "area": "physics"},

    # Scientific Discovery
    {"topic": "AI for scientific discovery: hypothesis generation", "area": "discovery"},
    {"topic": "Automated experimentation: self-driving labs", "area": "discovery"},
    {"topic": "Literature mining: scientific paper analysis", "area": "discovery"},
    {"topic": "Knowledge extraction: scientific graphs", "area": "discovery"},

    # Simulation
    {"topic": "ML surrogates: fast simulation approximation", "area": "simulation"},
    {"topic": "Molecular dynamics ML: force field learning", "area": "simulation"},
    {"topic": "Weather simulation: ML weather models", "area": "simulation"},
    {"topic": "Fluid dynamics ML: CFD acceleration", "area": "simulation"},

    # Materials & Chemistry
    {"topic": "Materials discovery: property prediction", "area": "materials"},
    {"topic": "Crystal structure prediction: generative models", "area": "materials"},
    {"topic": "Reaction prediction: retrosynthesis AI", "area": "materials"},
    {"topic": "Catalyst design: ML for chemistry", "area": "materials"},

    # Platforms
    {"topic": "DeepMind scientific AI: AlphaFold, GNoME", "area": "platforms"},
    {"topic": "NVIDIA Modulus: physics ML framework", "area": "platforms"},
    {"topic": "JAX for science: differentiable computing", "area": "platforms"},
    {"topic": "PyTorch Geometric: graph neural networks", "area": "platforms"},
]

@dataclass
class ScientificMLResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class ScientificMLExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("scientific_ml", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Scientific ML Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Scientific machine learning: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("scientific_ml", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ScientificMLResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ScientificMLResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("SCIENTIFIC ML ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(SCIENTIFIC_ML_TOPICS)}")
    print("="*70)

    executor = ScientificMLExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(SCIENTIFIC_ML_TOPICS, 1):
        try:
    ...
    with open("scientific_ml_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to scientific_ml_results.json")
    print("="*70)
