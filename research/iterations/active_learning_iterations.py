"""
ACTIVE LEARNING ITERATIONS - Efficient Labeling
================================================
Active learning, data efficiency, sampling
"""
...
ACTIVE_LEARNING_TOPICS = [
    # Strategies
    {"topic": "Uncertainty sampling: least confident, margin", "area": "strategies"},
    {"topic": "Query by committee: disagreement sampling", "area": "strategies"},
    {"topic": "Expected model change: gradient information", "area": "strategies"},
    {"topic": "Diversity sampling: representative selection", "area": "strategies"},

    # Deep Active Learning
    {"topic": "Deep active learning: neural network selection", "area": "deep"},
    {"topic": "Bayesian active learning: uncertainty estimation", "area": "deep"},
    {"topic": "Core-set selection: geometry-based", "area": "deep"},
    {"topic": "Learning loss prediction: meta-learning", "area": "deep"},

    # Applications
    {"topic": "Active learning NLP: text annotation", "area": "applications"},
    {"topic": "Active learning CV: image labeling", "area": "applications"},
    {"topic": "Medical active learning: expert annotation", "area": "applications"},
    {"topic": "Active learning production: labeling pipelines", "area": "applications"},

    # Techniques
    {"topic": "Batch active learning: selecting multiple samples", "area": "techniques"},
    {"topic": "Multi-task active learning: shared selection", "area": "techniques"},
    {"topic": "Active learning stopping: when to stop", "area": "techniques"},
    {"topic": "Human-in-the-loop: annotation interfaces", "area": "techniques"},

    # Tools
    {"topic": "Prodigy: annotation tool, active learning", "area": "tools"},
    {"topic": "Label Studio: ML annotation platform", "area": "tools"},
    {"topic": "ModAL: modular active learning Python", "area": "tools"},
    {"topic": "ALiPy: active learning toolbox", "area": "tools"},
]
...
class ActiveLearningExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("active_learning", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Active Learning Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Active learning ML: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("active_learning", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> ActiveLearningResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return ActiveLearningResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)
...
async def main():
    ...
    print(f"Topics: {len(ACTIVE_LEARNING_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(ACTIVE_LEARNING_TOPICS, 1):
        try:
    ...
    with open("active_learning_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to active_learning_results.json")
    print("="*70)
