"""
NLP TASKS ITERATIONS - Natural Language Processing
==================================================
NLP tasks, text classification, NER, summarization
"""
...
NLP_TASKS_TOPICS = [
    # Classification
    {"topic": "Text classification: sentiment analysis", "area": "classification"},
    {"topic": "Intent classification: chatbot NLU", "area": "classification"},
    {"topic": "Topic classification: document categorization", "area": "classification"},
    {"topic": "Zero-shot classification: no training", "area": "classification"},

    # Extraction
    {"topic": "Named entity recognition: NER", "area": "extraction"},
    {"topic": "Relation extraction: knowledge graphs", "area": "extraction"},
    {"topic": "Keyword extraction: key phrases", "area": "extraction"},
    {"topic": "Information extraction: structured data", "area": "extraction"},

    # Generation
    {"topic": "Text summarization: abstractive extractive", "area": "generation"},
    {"topic": "Machine translation: multilingual", "area": "generation"},
    {"topic": "Question answering: reading comprehension", "area": "generation"},
    {"topic": "Text generation: creative writing", "area": "generation"},

    # Understanding
    {"topic": "Semantic similarity: text matching", "area": "understanding"},
    {"topic": "Natural language inference: NLI", "area": "understanding"},
    {"topic": "Coreference resolution: entity linking", "area": "understanding"},
    {"topic": "Dependency parsing: syntax analysis", "area": "understanding"},

    # Applications
    {"topic": "Chatbots: conversational NLP", "area": "applications"},
    {"topic": "Content moderation: toxic detection", "area": "applications"},
    {"topic": "Search relevance: ranking", "area": "applications"},
    {"topic": "Document processing: enterprise NLP", "area": "applications"},
]

@dataclass
class NLPTasksResult:
    topic: str
    area: str
    sources: list
    findings: list
...
class NLPTasksExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("nlp_tasks", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] NLP Tasks Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"NLP natural language processing: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("nlp_tasks", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> NLPTasksResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return NLPTasksResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("NLP TASKS ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(NLP_TASKS_TOPICS)}")
    print("="*70)

    executor = NLPTasksExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(NLP_TASKS_TOPICS, 1):
        try:
    ...
    with open("nlp_tasks_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to nlp_tasks_results.json")
    print("="*70)
