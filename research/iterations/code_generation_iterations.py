...
class CodeExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("code", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Code Generation Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"AI code generation: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("code", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> CodeResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return CodeResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)
...
async def main():
    ...
    print(f"Topics: {len(CODE_TOPICS)}")
    print("="*70)
    ...
    for i, topic_data in enumerate(CODE_TOPICS, 1):
        try:
    ...
    print(f"\n[OK] Saved to code_generation_results.json")
    print("="*70)
