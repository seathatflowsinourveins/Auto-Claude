"""
CODE LLM ITERATIONS - Code Generation Models
=============================================
Code LLMs, code generation, programming AI
"""
...
CODE_LLM_TOPICS = [
    # Models
    {"topic": "Codex: OpenAI code model", "area": "models"},
    {"topic": "CodeLlama: Meta code LLM", "area": "models"},
    {"topic": "StarCoder: BigCode open model", "area": "models"},
    {"topic": "DeepSeek Coder: code generation", "area": "models"},

    # Training
    {"topic": "Code pretraining: GitHub data", "area": "training"},
    {"topic": "Fill-in-middle: FIM training", "area": "training"},
    {"topic": "Instruction tuning: code tasks", "area": "training"},
    {"topic": "RLHF code: human feedback", "area": "training"},

    # Capabilities
    {"topic": "Code completion: autocomplete", "area": "capabilities"},
    {"topic": "Code generation: natural language", "area": "capabilities"},
    {"topic": "Code explanation: documentation", "area": "capabilities"},
    {"topic": "Bug fixing: automated repair", "area": "capabilities"},

    # Applications
    {"topic": "GitHub Copilot: AI pair programming", "area": "applications"},
    {"topic": "Cursor: AI-first IDE", "area": "applications"},
    {"topic": "Codeium: free code assistant", "area": "applications"},
    {"topic": "Replit AI: cloud coding", "area": "applications"},

    # Evaluation
    {"topic": "HumanEval: code benchmark", "area": "evaluation"},
    {"topic": "MBPP: programming problems", "area": "evaluation"},
    {"topic": "CodeContests: competitive programming", "area": "evaluation"},
    {"topic": "SWE-bench: real-world issues", "area": "evaluation"},
]
...
class CodeLLMExecutor:
    ...
    async def initialize(self):
        ...
        self.qdrant.create_collection("code_llm", vectors_config=VectorParams(size=1024, distance=Distance.COSINE))
        print("[OK] Code LLM Executor initialized")
    ...
    async def _perplexity(self, client, topic: str, area: str) -> dict:
        try:
            r = await client.post('https://api.perplexity.ai/chat/completions', headers={'Authorization': f'Bearer {self.keys["perplexity"]}'}, json={'model': 'sonar-pro', 'messages': [{'role': 'user', 'content': f"Code LLM programming: {topic}"}], 'return_citations': True})
            data = r.json()
    ...
    async def _embed(self, client, texts: list) -> int:
        try:
            ...
            self.qdrant.upsert("code_llm", points=points)
            return len(points)
        except: return 0

    async def run_iteration(self, topic_data: dict, index: int) -> CodeLLMResult:
        ...
        if result["findings"]:
            ...
            print(f"    -> {clean[:60]}...")
        return CodeLLMResult(topic=topic, area=area, sources=result["sources"], findings=result["findings"], vectors=result["vectors"], latency=latency)

async def main():
    print("="*70)
    print("CODE LLM ITERATIONS")
    print("="*70)
    print(f"Start: {datetime.now().isoformat()}")
    print(f"Topics: {len(CODE_LLM_TOPICS)}")
    print("="*70)

    executor = CodeLLMExecutor()
    await executor.initialize()
    all_results = []

    for i, topic_data in enumerate(CODE_LLM_TOPICS, 1):
        try:
    ...
    with open("code_llm_results.json", 'w') as f:
    ...
    print(f"\n[OK] Saved to code_llm_results.json")
    print("="*70)
